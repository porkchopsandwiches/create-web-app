    ...
    return (nextValue: T) => {
        if (!list.includes(nextValue)) {
            throw new commander.InvalidOptionArgumentError(
                `"${nextValue}" not valid, must be one of ${list.join(", ")}`,
            );
        }
        return nextValue;
    };
...
export const buildProgram = (): ProgramAndParse => {
    ...
    program
        ...
        .option(
            ...
            "Package manager to use for scripts, 'npm' or 'pnpm'",
            validateFromListFactory(validPackageManagers),
            validPackageManagers[0],
        )
        .option(
            "-s, --state-library <stateLibrary>",
            "State library to use, 'none' or 'recoil'",
            validateFromListFactory(validStateLibraries),
            validStateLibraries[0],
        )
